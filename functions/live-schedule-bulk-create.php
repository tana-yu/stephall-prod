...
add_action('admin_menu', function () {
    add_submenu_page(
        ...
        '未公開Event一括作成',
        'manage_options',
        'bulk-create-live-schedule',
        'render_live_schedule_bulk_page'
    );
});

// ページ内容
function render_live_schedule_bulk_page() {
    if (isset($_POST['year'], $_POST['month'])) {
        $year  = (int) $_POST['year'];
        $month = (int) $_POST['month'];
        $count = create_unpublished_event_posts($year, $month);
        echo '<div class="updated"><p>' . esc_html($count) . ' 件の「未公開 Event」を作成しました。</p></div>';
    }
    ?>
    <div class="wrap">
        <h1>未公開 Event 一括作成</h1>
        <form method="post">
...
// 投稿作成処理
function create_unpublished_event_posts($year, $month) {
    ...
    for ($day = 1; $day <= $days_in_month; $day++) {
        ...
        $post_id = wp_insert_post([
            ...
            'post_name'   => sanitize_title('未公開 Event'),
            'post_type'   => 'live-schedule',
            'post_status' => 'publish',
            'post_content'=> '',
        ]);
